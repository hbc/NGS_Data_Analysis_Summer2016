...
Now that we have a sorted BAM file that has duplicates marked, let's create an index for our BAM file so we can eventually visualize our data in IGV. As we have done in previous sessions, we will use *Samtools* to create the index:
